...
Manifest = {
    ...
    'Version': '1.1',
    'Author': 'Aruba Networks and modified by Paul Kim'
}

ParameterDefinitions = {
    'connectivity_check_rate': {
        'Name': 'Connectivity Check Rate (in seconds)',
    ...
    'ipsla_destination': {
        'Name': 'IP-SLA Destination (x.x.x.x/x)',
        'Description': (''),
        'Type': 'string',
        'Default': '123.123.123.0/24'
    },
    'ipsla_nexthop': {
        'Name': 'IP-SLA Next Hop (x.x.x.x)',
        'Description': (''),
        'Type': 'string',
        'Default': '10.10.10.1'
    }
...
class Agent(NAE):
    ...
    def create_monitor(self):
        ...
        self.r1.condition('every {} seconds', [
        ...
        rate_time_interval = check_rate + ' seconds'
    ...
    def remove_alert(self):
        ...
        if (alert_level == AlertLevel.CRITICAL) or \
            ...
            #edit by paul.kim
            cmd = "config\nip route %s %s\nexit" % (self.params['ipsla_destination'], self.params['ipsla_nexthop'])
            ActionCLI(cmd)
